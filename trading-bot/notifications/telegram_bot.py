...
def format_setup_message(
    ...
    """
    pair   = setup.get("pair", "?").replace("USDT", "/USDT")
    ...
    return (
        f"🎯 <b>TRADING SETUP — {pair}</b>\n"
        f"\n"
        f"📊 <b>BIAS:</b> {bias} {bias_arrow}\n"
        f"⏱ Timeframe: 15m entry | 4H struktūra\n"
        f"\n"
        f"💰 <b>POZICIJA:</b>\n"
        f"  Entry:  <code>${entry:,.2f}</code>\n"
        f"  SL:     <code>${sl:,.2f}</code>  ({sl_sign}{sl_pct:.1f}%)\n"
        f"  TP1:    <code>${tp1:,.2f}</code>  ({tp1_sign}{tp1_pct:.1f}%)\n"
        f"  TP2:    <code>${tp2:,.2f}</code>\n"
        f"  R:R  =  1:{rr:.1f}\n"
        f"\n"
        f"💼 <b>RIZIKA</b> (Level {level_num} | Balance: ${balance}):\n"
        ...
        f"  OB zona: {ob_range} ✓\n"
        f"  FVG: {fvg_label}\n"
        f"  Ross Hook: {hook_label}\n"
        f"\n"
        f"⚠️ <b>INVALIDATION:</b> 4H close {'žemiau' if bias == 'LONG' else 'virš'} "
        f"${sl:,.2f}\n"
        f"\n"
        f"💬 <i>{reasoning}</i>\n"
        f"\n"
